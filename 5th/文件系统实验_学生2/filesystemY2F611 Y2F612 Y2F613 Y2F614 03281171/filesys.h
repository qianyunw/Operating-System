...
#define MAX_DINODEID (BLOCKSIZ/DINODESIZ*DINODEBLK)
...
struct file{
	unsigned short f_flag;    /*文件操作标志*/	//{}char f_flag;
...
extern char   disk[(DINODEBLK+FILEBLK+2)*BLOCKSIZ];		/*整个磁盘大小*/

extern struct inode *iget();
extern iput(struct inode *pinode);
extern unsigned int balloc();
extern bfree();
extern struct inode *ialloc();
extern ifree(unsigned int dinodeid);
...
extern mkdir();
extern chdir();
extern unsigned short open(unsigned uid, char *filename, unsigned short openmode);
extern creat(unsigned short uid, char *filename, unsigned short mode);
extern close(unsigned int user_id, unsigned short cfd);
extern delete(char *filename);
...
extern int uid_to_userid(unsigned short uid);
extern void testmkdir();
extern void testiallfre();
extern void testballoc();
